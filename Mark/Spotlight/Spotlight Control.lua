...
function onLoad(save_state)
	...
	self.createButton({
		click_function = "OnLight",
		function_owner = self,
		label = "Show All Light",
		position = {-3.8, 0, 0.15},
		rotation = {0, 90, 0},
		scale = {0.5, 0.5, 0.5},
		width = 2300,
		height = 400,
		font_size = 300,
		tooltip = "Show the light on every spotlight tool in the scene.",
		color = Color.fromString("Green"):lerp(Color.fromString("White"), 0.7),
	})
	self.createButton({
		click_function = "OffLight",
		function_owner = self,
		label = "Hide All Light",
		position = {-4.2, 0.030, 0.15},
		rotation = {0, 90, 0},
		scale = {0.5, 0.5, 0.5},
		width = 2300,
		height = 400,
		font_size = 300,
		tooltip = "Hide the light on every spotlight tool in the scene.",
		color = Color.fromString("Red"):lerp(Color.fromString("White"), 0.7),
	})
	self.createButton({
...
function OnLight(obj, color, alt)
	for _, obj in ipairs(getAllObjects()) do
		if obj.getName() == "Custom Spotlight Tool" then
			obj.call("apply", "On")
		end
...
function OffLight(obj, color, alt)
	for _, obj in ipairs(getAllObjects()) do
		if obj.getName() == "Custom Spotlight Tool" then
			obj.call("apply", "Off")
		end
